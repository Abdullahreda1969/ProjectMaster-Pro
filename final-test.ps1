...
try {
    ...
    Write-Host "   📊 قاعدة البيانات: $($health.database.connected ? '✅ متصلة' : '❌ غير متصلة')" -ForegroundColor White
} catch {
...
1..3 | ForEach-Object {
    try {
        ...
        Write-Host "   ✅ المشروع $_: $($result.project.code)" -ForegroundColor Green
    } catch {
        Write-Host "   ❌ فشل المشروع $_: $($_.Exception.Message)" -ForegroundColor Red
    }
